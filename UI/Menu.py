...
class Menu(tk.Frame):
	def __init__(self, master, name=None, vs=None):
		...
		tk.Button(self, text='Seated Forward Bend', bg='#DDDDFF', font=('Comic Sans MS', 14), activeforeground='#ADADAD',
			command=lambda: master.switch_frame(TeachStage, 'Seated Forward Bend', self.vs)).place(x=720, y=350, relwidth=0.17, relheight=0.08)
		tk.Button(self, text='Low Lunge', bg='#DDDDFF', font=('Comic Sans MS', 14), activeforeground='#ADADAD',
			command=lambda: master.switch_frame(TeachStage, 'Low Lunge', self.vs)).place(x=320, y=450, relwidth=0.17, relheight=0.08)
		tk.Button(self, text='Downward dog', bg='#DDDDFF', font=('Comic Sans MS', 14), activeforeground='#ADADAD',
			command=lambda: master.switch_frame(TeachStage, 'Downward dog', self.vs)).place(x=720, y=450, relwidth=0.17, relheight=0.08)
		tk.Button(self, text='Pyramid pose', bg='#DDDDFF', font=('Comic Sans MS', 14), activeforeground='#ADADAD',
			command=lambda: master.switch_frame(TeachStage, 'Pyramid pose', self.vs)).place(x=320, y=550, relwidth=0.17, relheight=0.08)
		tk.Button(self, text='Bridge pose', bg='#DDDDFF', font=('Comic Sans MS', 14), activeforeground='#ADADAD',
			command=lambda: master.switch_frame(TeachStage, 'Bridge pose', self.vs)).place(x=720, y=550, relwidth=0.17, relheight=0.08)
